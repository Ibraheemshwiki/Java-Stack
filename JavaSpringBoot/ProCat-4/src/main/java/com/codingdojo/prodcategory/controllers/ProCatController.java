...
public class ProCatController {
	...
	public String categoryShow(Model model, @PathVariable("id")Long id,@ModelAttribute("product") Product product) {
		model.addAttribute("thisCategoryId", id);
		model.addAttribute("allCategoryProducts", proService.getCategoryProducts(id));
	...
	public String addRelation2(@PathVariable("id")Long id,@RequestParam("cid")Long cid) {
		proService.addCategoryProduct(id, cid);
